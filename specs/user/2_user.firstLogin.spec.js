...
describe.skip('USER LOGIN main Test Suite', () => {
  //
  describe('SMOKE TESTS', () => {
    describe(`  POSITIVE - happy path with valid credentials (EMAIL is "${process.env.USEREMAIL} and PASSWORD is "${process.env.USERPASSWORD}")`, () => {
      before(async () => {
    ...
    describe(`  NEGATIVE - unhappy path with invalid credentials (EMAIL is "${localHelper.testPath.user.login.error.email} and PASSWORD is "${localHelper.testPath.user.login.error.password}")`, () => {
      before(async () => {
        await userHelper.login(
          localHelper.testPath.user.login.error.email,
          localHelper.testPath.user.login.error.password
        );
        //console.log(userHelper.response.statusCode);
        //console.log(userHelper.response.body);
      });
      it(`Checking that response.statusCode is "${localHelper.testPath.user.login.error.statusCode}"`, () => {
        expect(userHelper.response.statusCode).to.be.eq(
          localHelper.testPath.user.login.error.statusCode
        );
      });
      it(`Checking that response.body.message is "${localHelper.testPath.user.login.error.bodyMessage}"`, () => {
        expect(userHelper.response.body.message).to.be.eq(
  ...
  describe(`Test Suite for all the detail Test Cases after successfull login (EMAIL is "${process.env.USEREMAIL} and PASSWORD is "${process.env.USERPASSWORD}")`, () => {
    ...
    describe(`Checking that before email was confirmed new user has role "NEW"`, () => {
      ...
      it(`Checking that response.body.payload.user.roles has only 1 role that is "new"`, () => {
